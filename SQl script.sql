...
round(
100.0 *
sum(case when discount_applied = 'Yes' then 1 else 0 end)/ count(*)
,2) as discount_rate
from customer
group by 1
order by 2 desc 
...
with cte as (
    select category, item_purchased,
           ...
           row_number() over (partition by category order by count(customer_id) desc) as row_rank
    from customer
    group by 1, 2
)
select row_rank, category, item_purchased, total_orders
from cte
where row_rank <=3
